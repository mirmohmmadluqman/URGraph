...
export function LifeGraphHeader() {
  const { resetData, actions, importData } = useLifeGraph();
  ...
  const handleExport = (format: 'json' | 'csv' | 'png' | 'svg') => {
    if (format === 'json') {
      ...
      link.href = url;
      link.download = 'lifegraph_data.json';
      link.click();
      ...
      const link = document.createElement('a');
      link.href = url;
      link.download = 'lifegraph_data.csv';
      link.click();
      URL.revokeObjectURL(url);
    } else {
        const svg = document.querySelector('#lifegraph-chart-svg');
        if (!svg) {
  ...
  return (
    <header className="flex items-center justify-between p-4 border-b border-black/10 dark:border-white/10 backdrop-blur-sm bg-white/30 dark:bg-black/30 sticky top-0 z-10">
      <div className="flex items-center gap-2">
        ...
        <h1 className="text-2xl font-bold tracking-tighter">LifeGraph</h1>
      </div>
      <div className="flex items-center gap-2">
