...
export function LifeGraphProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    try {
      const storedActions = localStorage.getItem('lifegraph:actions')
      if (storedActions) {
  ...
  useEffect(() => {
    try {
      localStorage.setItem('lifegraph:actions', JSON.stringify(actions))
    } catch (error)
    {
  ...
  useEffect(() => {
    try {
        localStorage.setItem('lifegraph:goal', JSON.stringify(goal));
    } catch (error) {
  ...
  const resetData = useCallback(() => {
    ...
    toast({
      title: "Data Reset",
      description: "All your LifeGraph data has been cleared.",
    })
  }, [toast])
  ...
  return (
    <LifeGraphContext.Provider value={value}>
      {children}
    </LifeGraphContext.Provider>
  )
}
